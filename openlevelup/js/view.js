...
//CONSTRUCTOR
	...
	if(bbox != undefined) {
		...
		if(coordinates.length == 4) {
			...
			this._mainView.getMessagesView().displayMessage("無效的邊界", "alert");
	...
	FeatureView.prototype.createPopup = function() {
		...
		if(style.levelup && ftLevels.length > 0 && !isMobile) {
			...
			if(levelId < ftLevels.length -1) {
				text += ' <a onclick="controller.toLevel('+ftLevels[levelId+1]+')" href="#"><img src="'+CONFIG.view.icons.folder+'/arrow_up_3.png" title="上樓" alt="上!" /></a>';
			}
			//Able to go down ?
			if(levelId > 0) {
				text += ' <a onclick="controller.toLevel('+ftLevels[levelId-1]+')" href="#"><img src="'+CONFIG.view.icons.folder+'/arrow_down_3.png" title="下樓" alt="下!" /></a>';
			}
		}
